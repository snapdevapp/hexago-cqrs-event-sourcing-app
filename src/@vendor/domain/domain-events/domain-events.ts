...
export class DomainEvents {
  ...
  public static subscribe<T extends DomainEventHandler>(event: DomainEventClass, eventHandler: T): void {
  ...
  public static async publishEvents(id: ID, logger: LoggerPort, correlationId?: string): Promise<void> {
  ...
  private static removeAggregateFromPublishList(aggregate: AggregateRoot<unknown>): void {
    const index = this.aggregates.findIndex(a => a.equals(aggregate));
    this.aggregates.splice(index, 1);
  }

  private static async publish(event: DomainEvent, logger: LoggerPort): Promise<void> {
